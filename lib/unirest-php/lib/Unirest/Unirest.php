...
class Unirest
{
    ...
    {
        return static::request(HttpMethod::GET, $url, $parameters, $headers, $username, $password);
    }
    ...
    {
        return static::request(HttpMethod::POST, $url, $body, $headers, $username, $password);
    }
    ...
    {
        return static::request(HttpMethod::DELETE, $url, $body, $headers, $username, $password);
    }
    ...
    {
        return static::request(HttpMethod::PUT, $url, $body, $headers, $username, $password);
    }
    ...
    {
        return static::request(HttpMethod::PATCH, $url, $body, $headers, $username, $password);
    }
    ...
    protected static function request($httpMethod, $url, $body = NULL, $headers = array(), $username = NULL, $password = NULL)
    {
        ...
        $finalHeaders = array_merge($headers, static::$defaultHeaders);
        foreach ($finalHeaders as $key => $val) {
            $lowercaseHeaders[] = static::getHeader($key, $val);
        }
        ...
        if ($httpMethod != HttpMethod::GET) {
            ...
            if (is_array($body) || $body instanceof Traversable) {
                static::http_build_query_for_curl($body, $postBody);
        ...
        curl_setopt($ch, CURLOPT_URL, static::encodeUrl($url));
        ...
        if (static::$socketTimeout != null) {
            curl_setopt($ch, CURLOPT_TIMEOUT, static::$socketTimeout);
        }
        if (!empty($username)) {
    ...
    protected static function getArrayFromQuerystring($querystring)
    {
    ...
    protected static function encodeUrl($url)
    {
        ...
        if ($query != null) {
            $query = '?' . http_build_query(static::getArrayFromQuerystring($url_parsed['query']));
        }
    ...
    protected static function getHeader($key, $val)
    {
